...
export default class StudentDataTable extends React.Component {
	...
	getStudentMergedFile(studentId, deptId, studentNo, studentName, deptName) {
		window.API.getStudentMergedFile(window.getSystem(), studentId, deptId, studentNo, studentName, deptName);
	}
								...
								studentList.map((val, i) => {
									return (
										<tr key={`${val.no}-${val.name}-${val.dept}`}>
											...
											<td className="text-center"><Button color="success" size="sm" onClick={()=>{ alert('功能修復中'); }}>下載審查資料</Button></td>
										</tr>
									);
								})
							}
